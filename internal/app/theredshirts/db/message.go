...
const (
	...
	select_first_messages_of_player     = "SELECT id, send_time, lobby_id, player_id, number, topic, message FROM %s.%s WHERE lobby_id = $1 AND player_id != $2 AND number > (SELECT number FROM %s.%s WHERE lobby_id = $1 AND player_id = $2 AND topic = 'PLAYER_JOINS_LOBBY')"
